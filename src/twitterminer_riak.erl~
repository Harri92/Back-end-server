...
-export([twitter_example/2, twitter_save_pipeline/5, get_riak_hostport/1
	...
	, initiate_bucket_purge/0, print/1]). %gather:tweet
...
test(N) -> io:format("This iss gor test reasons ~p",[N]).
...
lists:foreach(fun(X) -> io:format("~p has been deleted from gayharri.~n",[X]), delete_all_in_bucket(Pid, gayharri, X) end, element(2, riakc_pb_socket:list_keys(Pid, binary(gayharri)))).
...
sort_to_tweet(_, _, {parsed_tweet, [{_, [{L}]}], _Tweet_body, no_id}) -> case lists:keyfind(<<"code">>, 1, L) of
						{_, 88} -> lists:foreach(fun(X) -> io:format("~p minutes remaining ..~n",[X]), timer:sleep(60000) end, lists:reverse(lists:seq(1, 15)));
